...
def compute_norm(arr: np.ndarray, norm_type: str) -> float:
    ...
    if (norm_type == 'l1'):
        return float(np.sum(np.abs(arr.flatten())))
    elif (norm_type == 'l2'):
        return float(np.sqrt(np.sum(arr.flatten() * arr.flatten())))
    elif (norm_type == 'linf'):
        return float(np.max(np.abs(arr.flatten())))
    elif (norm_type == 'frobenius'):
        if (arr.ndim != 2):
            raise ValueError('Expect 2d array for frobenius')
